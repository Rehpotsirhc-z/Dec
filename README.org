...
install() {
    ...
    TOINSTALL="$(diff --new-line-format="" --unchanged-line-format="" <(sort -u "$PACKAGES" | sed -e 's/#.*//g' -e '/^$/d' -e 's/^[[:blank:]]*//' -e 's/[[:blank:]]*$//') <(paru -Qqe | sort -u) | tr "\n" " ")"
    if [[ -n "$TOINSTALL" ]]; then
...
remove() {
    ...
    TOREMOVE="$(diff --new-line-format="" --unchanged-line-format="" <(paru -Qqett | sort -u) <(sort -u "$PACKAGES" | sed -e 's/#.*//g' -e '/^$/d' -e 's/^[[:blank:]]*//' -e 's/[[:blank:]]*$//') | tr "\n" " ")"
    if [[ -n "$TOREMOVE" ]]; then
